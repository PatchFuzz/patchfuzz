...
for ( let memtype of memtypes ) {
    ...
    print(() => serialize(mem1),
		       TypeError,
		       /unsupported type for structured data/);
    print(() => serialize(mem1, [mem1]),
		       TypeError,
...
for ( let memtype of memtypes ) {
    ...
    print(mem2 instanceof WebAssembly.Memory, true);
    let buf2 = mem2.buffer;
    print(buf2 instanceof SharedArrayBuffer, true);

    print(buf1 !== buf2, true);
    print(buf1.byteLength, buf2.byteLength);
    if (memtype != '' && mem2.type) {
        print(mem2.type().index, memtype);
    }
    ...
    v1[37] = 0x12345678;
    print(v2[37], 0x12345678);

    

    let index = 2*65536 + 200;
    let access = print(`(module
    ...
    print(access, WebAssembly.RuntimeError, /out of bounds/);
