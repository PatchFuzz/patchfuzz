...
for (let type of ['i32', 'f32', 'f64']) {
    ...
    function main() {
        ...
        for (var i = 0; i < ITER; i++) {
            ...
            printPreciseStacks(endProfiling(), [INLINED_CALL_STACK, FAST_ENTRY_STACK, SLOW_ENTRY_STACK]);
            ...
            let which = printPreciseStacks(stack, [FAST_OOL_ENTRY_STACK, SLOW_ENTRY_STACK, EXCEPTION_ENTRY_STACK]);
            if (which !== null) {
                if (i === EXCEPTION_ITER) {
                    print(which, 2);
                }
        ...
        print(sum === 0 || sum === ITER, true);
        if (sum === ITER) {
            print(observedStacks[0] > 0, true, "the fast entry should have been taken at least once");
            print(observedStacks[2], 1, "the error path should have been taken exactly once");
        }
    }
