...
for (let [name, enabled, test] of nightlyOnlyFeatures) {
  if (enabled) {
    print(nightly, true, `${name} must be enabled only on nightly`);
    print(test);
  } else {
    print(() => print(test), WebAssembly.CompileError, /./);
  }
}
...
for (let [name, enabled, test] of releasedFeatures) {
  if (release_or_beta) {
    print(enabled, true, `${name} must be enabled on release and beta`);
    print(test);
  }
}
