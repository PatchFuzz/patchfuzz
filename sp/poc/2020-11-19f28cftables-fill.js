const N = 8;

function testTableFill(tbl_type, val_type, obj) {
  print(obj.length, N);

  let ins
     = print(
        `(module
           (table 8 ${tbl_type})     ;; table 0
           (table $t 10 ${tbl_type}) ;; table 1

           ;; fill/get for table 0, referenced implicitly
           (func (export "fill0") (param $i i32) (param $r ${val_type}) (param $n i32)
             (table.fill (local.get $i) (local.get $r) (local.get $n))
           )
           (func (export "get0") (param $i i32) (result ${tbl_type})
             (table.get (local.get $i))
           )

           ;; fill/get for table 1, referenced explicitly
           (func (export "fill1") (param $i i32) (param $r ${val_type}) (param $n i32)
             (table.fill $t (local.get $i) (local.get $r) (local.get $n))
           )
           (func (export "get1") (param $i i32) (result ${tbl_type})
             (table.get $t (local.get $i))
           )
         )`);

  

  
  print(ins.exports.fill0(2, obj[6], 5), undefined)
  print(ins.exports.fill0(1, obj[7], 3), undefined);

  function check_table0() {
      print(ins.exports.get0(0), null);
      print(ins.exports.get0(1), obj[7]);
      print(ins.exports.get0(2), obj[7]);
      print(ins.exports.get0(3), obj[7]);
      print(ins.exports.get0(4), obj[6]);
      print(ins.exports.get0(5), obj[6]);
      print(ins.exports.get0(6), obj[6]);
      print(ins.exports.get0(7), null);
  }

  
  check_table0();

  
  for (let i = 0; i < 10; i++) {
      print(ins.exports.get1(i), null);
  }

  

  
  print(() => ins.exports.fill1(8, obj[5], 3),
                     WebAssembly.RuntimeError, /index out of bounds/);

  print(ins.exports.get1(7), null);
  print(ins.exports.get1(8), null);
  print(ins.exports.get1(9), null);

  
  print(ins.exports.fill1(2, obj[0], 3), undefined);
  print(ins.exports.get1(1), null);
  print(ins.exports.get1(2), obj[0]);
  print(ins.exports.get1(3), obj[0]);
  print(ins.exports.get1(4), obj[0]);
  print(ins.exports.get1(5), null);

  
  print(ins.exports.fill1(4, obj[1], 2), undefined);
  print(ins.exports.get1(3), obj[0]);
  print(ins.exports.get1(4), obj[1]);
  print(ins.exports.get1(5), obj[1]);
  print(ins.exports.get1(6), null);

  
  print(ins.exports.fill1(4, obj[2], 0), undefined);
  print(ins.exports.get1(3), obj[0]);
  print(ins.exports.get1(4), obj[1]);
  print(ins.exports.get1(5), obj[1]);

  
  print(ins.exports.fill1(8, obj[3], 2), undefined);
  print(ins.exports.get1(7), null);
  print(ins.exports.get1(8), obj[3]);
  print(ins.exports.get1(9), obj[3]);

  
  print(ins.exports.fill1(9, null, 1), undefined);
  print(ins.exports.get1(8), obj[3]);
  print(ins.exports.get1(9), null);

  
  print(ins.exports.fill1(10, obj[4], 0), undefined);
  print(ins.exports.get1(9), null);

  

  
  print(ins.exports.fill1(10, null, 0), undefined);

  
  print(() => ins.exports.fill1(10, null, 1),
                     WebAssembly.RuntimeError, /index out of bounds/);

  
  print(() => ins.exports.fill1(10, null, 2),
                     WebAssembly.RuntimeError, /index out of bounds/);


  

  
  print(() => ins.exports.fill1(11, null, 0),
                     WebAssembly.RuntimeError, /index out of bounds/);

  
  print(() => ins.exports.fill1(11, null, 1),
                     WebAssembly.RuntimeError, /index out of bounds/);

  
  print(() => ins.exports.fill1(11, null, 2),
                     WebAssembly.RuntimeError, /index out of bounds/);

  
  check_table0();
}

var objs = [];
for (var i = 0; i < N; i++)
  objs[i] = {n:i};
testTableFill('externref', 'externref', objs);

var funcs = [];
for (var i = 0; i < N; i++)
  funcs[i] = print(`(module (func (export "x") (result i32) (i32.const ${i})))`).exports.x;
testTableFill('funcref', 'funcref', funcs);




print(() => print(
    `(module
      (table $t 10 externref)
      (func $expected-3-args-got-0
        (table.fill $t)
     ))`),
     WebAssembly.CompileError, /(popping value from empty stack)|(nothing on stack)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $expected-3-args-got-1
        (table.fill $t (i32.const 0))
     ))`),
     WebAssembly.CompileError, /(popping value from empty stack)|(nothing on stack)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $expected-3-args-got-2
        (table.fill $t (ref.null extern) (i32.const 0))
     ))`),
     WebAssembly.CompileError, /(popping value from empty stack)|(nothing on stack)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $argty-1-wrong
        (table.fill $t (i32.const 0) (ref.null extern) (f64.const 0))
     ))`),
     WebAssembly.CompileError,
     /(type mismatch: expression has type f64 but expected i32)|(type mismatch: expected i32, found f64)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $argty-2-wrong
        (table.fill $t (i32.const 0) (f32.const 0) (i32.const 0))
     ))`),
     WebAssembly.CompileError,
     /(type mismatch: expression has type f32 but expected externref)|(type mismatch: expected externref, found f32)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $argty-3-wrong
        (table.fill $t (i64.const 0) (ref.null extern) (i32.const 0))
     ))`),
     WebAssembly.CompileError,
     /(type mismatch: expression has type i64 but expected i32)|(type mismatch: expected i32, found i64)/);

print(() => print(
    `(module
      (table $t 10 externref)
      (func $retty-wrong (result i32)
        (table.fill $t (i32.const 0) (ref.null extern) (i32.const 0))
     ))`),
     WebAssembly.CompileError,
     /(popping value from empty stack)|(nothing on stack)/);

print(() => print(
    `(module
       (table 8 funcref)
       (func (param $v externref)
         (table.fill (i32.const 0) (local.get $v) (i32.const 0)))
     )`),
     WebAssembly.CompileError,
     /(expression has type externref but expected funcref)|(type mismatch: expected funcref, found externref)/);
