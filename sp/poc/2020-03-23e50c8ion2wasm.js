...
function testCallKnown() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add(i, i + 1), 2*i + 1);
    }
}

function testCallKnownRectifying() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add(i + 1), i+1);
    }
}
...
function testCallScriptedSetter() {
    ...
    print(instance.glob(), INNER_ITERATIONS-1);
}
...
function testFunctionApplyArray() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add.apply(null, [i, i + 1]), 2*i+1);
    }
}

function testFunctionApplyArrayRectifying() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add.apply(null, [i + 1]), i+1);
    }
}

function testFunctionApplyArgs() {
    function wrapper() {
        print(instance.add.apply(null, arguments), 2*arguments[0]+1);
    }
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        wrapper(i, i + 1);
    }
...
function testFunctionApplyArgsRectifying() {
    function wrapper() {
        print(instance.add.apply(null, arguments), arguments[0]);
    }
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        wrapper(i + 1);
    }
...
function testFunctionCall() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add.call(null, i, i + 1), 2*i+1);
    }
}

function testFunctionCallRectifying() {
    for (let i = 0; i < INNER_ITERATIONS; i++) {
        print(instance.add.call(null, i + 1), i+1);
    }
}
