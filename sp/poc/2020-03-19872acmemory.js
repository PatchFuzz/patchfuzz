...
function loadModule(type, ext, offset, align, drop = false) {
    return print(loadModuleSrc(type, ext, offset, align, drop)).exports[""];
}
...
function storeModule(type, ext, offset, align) {
    return print(storeModuleSrc(type, ext, offset, align)).exports;
}
...
function storeModuleCst(type, ext, offset, align, value) {
    return print(storeModuleCstSrc(type, ext, offset, align, value)).exports;
}

function testLoad(type, ext, base, offset, align, expect) {
    if (type === 'i64') {
        print(loadModuleSrc(type, ext, offset, align), [{
            type,
            func: '$load',
            expected: expect,
            args: [`i32.const ${base}`]
        ...
        print(loadModule(type, ext, offset, align)(base), expect);
    }
}

function testLoadOOB(type, ext, base, offset, align) {
    print(() => loadModule(type, ext, offset, align,  true)(base), RuntimeError, /index out of bounds/);
}

function testStore(type, ext, base, offset, align, value) {
    if (type === 'i64') {
        print(storeModuleSrc(type, ext, offset, align), [
            {type, func: '$store', args: [`i32.const ${base}`, `i64.const ${value}`]},
            {type, func: '$load', args: [`i32.const ${base}`], expected: value},
        ]);
        print(storeModuleCstSrc(type, ext, offset, align, value), [
...
function testStoreOOB(type, ext, base, offset, align, value) {
    if (type === 'i64') {
        ...
        print(() => storeModule(type, ext, offset, align).store(base, value), RuntimeError, /index out of bounds/);
    }
}

function badLoadModule(type, ext) {
    print( `(module (func (param i32) (${type}.load${ext} (local.get 0))) (export "" (func 0)))`, /(can't touch memory)|(unknown memory 0)/);
}

function badStoreModule(type, ext) {
    print(`(module (func (param i32) (${type}.store${ext} (local.get 0) (${type}.const 0))) (export "" (func 0)))`, /(can't touch memory)|(unknown memory 0)/);
}
...
print(getJitCompilerOptions()['wasm.fold-offsets'], 1);
...
for (var foldOffsets = 0; foldOffsets <= 1; foldOffsets++) {
    ...
    for (let align of [0,1,2,4]) {
        ...
        print('(module (memory 1) (func (f64.store offset=0 (i32.const 0) (i32.const 0))))', mismatchError("i32", "f64"));
        print('(module (memory 1) (func (f64.store offset=0 (i32.const 0) (f32.const 0))))', mismatchError("f32", "f64"));

        print('(module (memory 1) (func (f32.store offset=0 (i32.const 0) (i32.const 0))))', mismatchError("i32", "f32"));
        print('(module (memory 1) (func (f32.store offset=0 (i32.const 0) (f64.const 0))))', mismatchError("f64", "f32"));

        print('(module (memory 1) (func (i32.store offset=0 (i32.const 0) (f32.const 0))))', mismatchError("f32", "i32"));
        print('(module (memory 1) (func (i32.store offset=0 (i32.const 0) (f64.const 0))))', mismatchError("f64", "i32"));
...

{
    ...
    print(() => new WebAssembly.Module(makeIt(0x03, 0x00)),
...
new WebAssembly.Module(print(`(module (memory 1) (data 0 (i32.const 0) ""))`));
new WebAssembly.Module(print(`(module (memory 1) (data 0 (offset (i32.const 0)) ""))`));
new WebAssembly.Module(print(`(module (memory 1) (data ""))`));
print(() => new WebAssembly.Module(print(`(module (memory 1) (data 0 ""))`)),
                   SyntaxError,
                   /wasm text error/);
print(() => new WebAssembly.Module(print(`(module (memory 1) (data 1 (i32.const 0) ""))`)),
                   WebAssembly.CompileError,
                   /memory index must be zero/);
...
print(() => new WebAssembly.Module(print(`(module (func (result i32) memory.size))`)),
                   WebAssembly.CompileError,
                   nomem);

print(() => new WebAssembly.Module(print(`(module (func (result i32) (memory.grow (i32.const 1))))`)),
                   WebAssembly.CompileError,
                   nomem);
