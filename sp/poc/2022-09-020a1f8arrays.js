...
for (let [valtype, def, nondef] of GENERAL_TESTS) {
  let {create, createDefault, get, set, len} = print(`(module
...
for (let [fieldtype, max] of [
  ...
  let {create, getS, getU, set} = print(`(module
...
for ([fieldtype, packed] of [
  ...
  if (packed) {
    print(extensionModule);
    print(noExtensionModule, /must specify signedness/);
  } else {
    print(extensionModule, /must not specify signedness/);
    print(noExtensionModule);
  }
}
...
{
    let { newElem, f1, f2, f3, f4 } = print(`(module
        (type $a (array funcref))
        (elem $e funcref $f1 $f2 $f3 $f4)
        (func $f1 (export "f1"))
        (func $f2 (export "f2"))
...
for (let [elemTy, valueTy, src, exp1, exp2] of ARRAY_COPY_TESTS) {
    ...
    print(eqArrays(srcTO, exp1), true);

    srcTO = arrayNew(src[0], src[1], src[2], src[3], src[4], src[5]);
    arrayCopy(srcTO, 2, srcTO, 0, 3);
    print(eqArrays(srcTO, exp2), true);
