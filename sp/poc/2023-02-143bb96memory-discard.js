...
function checkRegion(exp, min, max, expectLetters) {
    for (let i = min; i < max; i++) {
        ...
        print(exp.letter(i), expected, `checking position of letter ${String.fromCharCode(c)}`);
    }
}
...
testAll(function testOOB(discardViaJS) {
    ...
    exp.init();
    checkWholeAlphabet(exp, true);
    print(() => discard(), WebAssembly.RuntimeError, /out of bounds/);
...
testAll(function testMisalignedStart(discardViaJS) {
    ...
    exp.init();
    checkWholeAlphabet(exp, true);
    print(() => discard(), WebAssembly.RuntimeError, /unaligned/);
