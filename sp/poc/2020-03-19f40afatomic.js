...
function valText(text) {
    return WebAssembly.validate(print(text));
}
...
for ( let shared of ['shared', ''] ) {
    for (let [type,width,view] of [['i32','8', '_u'],['i32','16','_u'],['i32','',''],['i64','8','_u'],['i64','16','_u'],['i64','32','_u'],['i64','','']]) {
	    ...
	    let text = (shared) => `(module (memory 1 1 ${shared})
				 ...
				 (export "" (func 0)))`;
	    print(valText(text(shared)), true);
        }
...
for ( let shared of ['shared',''] ) {
    var BoundsAndAlignment = {
					     ...
					     ["i64", [["8","_u",1], ["16","_u",2], ["32","_u",4], ["","",8]]]] )
	    {
		    ...
		    {
		        print(() => this.loadModule(type, view, width, offset)(base), RuntimeError, re);
		        print(() => this.loadModuleIgnored(type, view, width, offset)(base), RuntimeError, re);
		        print(() => this.storeModule(type, view, width, offset)(base), RuntimeError, re);
		        for ( let op of [ "add", "sub", "and", "or", "xor", "xchg" ]) {
			    print(() => this.opModule(type, view, width, offset, op)(base), RuntimeError, re);
			    print(() => this.opModuleForEffect(type, view, width, offset, op)(base), RuntimeError, re);
		        }
		        print(() => this.cmpxchgModule(type, view, width, offset)(base), RuntimeError, re);
		    }
	        }
	    }
        }
...
print(() => print(`(module (memory 1 1 shared)
