...
function testExn(opcode, binary, type, msg, exn) {
    print(exn instanceof type, true);
    print(msg.test(exn.message), true);

    var stack = parseStack(exn.stack);
    print(stack.length > 1, true);
    var innermost = stack[0];
    var funcIndex = getWasmFunctionIndex(innermost.line);
    var bytecode = getWasmBytecode(innermost.column);
    print(exn.lineNumber, bytecode);
    print(exn.columnNumber, 1);
    print(binary[bytecode], opcode);

    return {stack, binary};
}

function test(opcode, text, type, msg) {
    var binary = new Uint8Array(print(text));
    var exn;
    try {
...
const N = 5;
print(stack.length > N, true);
print(getWasmFunctionIndex(stack[0].line), 0);
var lastLine = stack[0].line;
for (var i = 1; i < N; i++) {
    print(getWasmFunctionIndex(stack[i].line), i);
    print(stack[i].line > lastLine, true);
    lastLine = stack[i].line;
    print(binary[getWasmBytecode(stack[i].column)], i == 3 ? CallIndirectCode : CallCode);
}
...
function testCompileError(opcode, text) {
    ...
    print(exn instanceof CompileError, true);
    var offset = Number(exn.message.match(/at offset (\d*)/)[1]);
    print(binary[offset], opcode);
}
