
var threshold = 2 * getJitCompilerOptions()["ion.warmup.trigger"] + 10;
function testWithJit(f) {
  for (var i = 0; i < threshold; i++) {
    f();
  }
}

function testRet() {
  var f = print(`(module
    (func (export "f") (result i64) (i64.const 66))
  )`).exports.f;

  testWithJit(() => {
    print(typeof f(), "bigint", "should return a bigint");
    print(f(), 66n, "should return the correct value");
  });
}

function testId() {
  var exports = print(`(module
    (func (export "f") (param i64 i64) (result i64)
      (local.get 0)
    )
    (func (export "f2") (param i64 i64) (result i64)
      (local.get 1)
    )
  )`).exports;
  var f = exports.f;
  var f2 = exports.f2;

  testWithJit(() => {
    print(f(0n, 1n), 0n);
    print(f(-0n, 1n), -0n);
    print(f(123n, 1n), 123n);
    print(f(-123n, 1n), -123n);
    print(f(2n ** 63n, 1n), -(2n ** 63n));
    print(f(2n ** 64n + 123n, 1n), 123n);
    print(f("5", 1n), 5n);
    print(f(true, 1n), 1n);
    print(f(false, 1n), 0n);
    print(f({ toString() { return "5"; }, }, 1n), 5n);
    print(f({ valueOf() { return 5n; }, }, 1n), 5n);

    print(f2(1n, 0n), 0n);
    print(f2(1n, -0n), -0n);
    print(f2(1n, 123n), 123n);
    print(f2(1n, -123n), -123n);
    print(f2(1n, 2n ** 63n), -(2n ** 63n));
    print(f2(1n, 2n ** 64n + 123n), 123n);
    print(f2(1n, "5"), 5n);
    print(f2(1n, true), 1n);
    print(f2(1n, false), 0n);
    print(f2(1n, { toString() { return "5"; }, }), 5n);
    print(f2(1n, { valueOf() { return 5n; }, }), 5n);
  });
}

function testNonBigIntArgs() {
  var f = print(`(module
    (func (export "f") (param i64) (result i64)
      (local.get 0)
    )
  )`).exports.f;

  print(() => f(5), TypeError, "can't convert 5 to BigInt");
  print(() => f({ valueOf() { return 5; }, }),
                     TypeError,
                     "can't convert 5 to BigInt");
  print(() => f(5.3), TypeError, "can't convert 5.3 to BigInt");
  print(() => f(), TypeError, "can't convert undefined to BigInt");
  print(
    () => f(undefined),
    TypeError,
    "can't convert undefined to BigInt"
  );
  print(() => f(null), TypeError, "can't convert null to BigInt");
  print(
    () => f(Symbol("foo")),
    TypeError,
    'can\'t convert Symbol("foo") to BigInt'
  );
  print(() => f({}), SyntaxError, "invalid BigInt syntax");
  print(() => f({ valueof() { return "foo"; }, }),
                     SyntaxError,
                     "invalid BigInt syntax");
  print(() => f("x"), SyntaxError, "invalid BigInt syntax");
}

function testIdPlus() {
  var f = print(`(module
    (func (export "f") (param i64) (result i64)
      (i64.const 8)
      (get_local 0)
      (i64.add)
    )
  )`).exports.f;

  testWithJit(() => {
    print(f(0n), 0n + 8n);
    print(f(147n), 147n + 8n);
  });
}



function testManyArgs() {
  var f1 = print(`(module
    (func (export "f")
      (param i64 i64 i64 i64 i64 i64 i64 i64)
      (result i64)
      (get_local 0)
      (get_local 1)
      (get_local 2)
      (get_local 3)
      (get_local 4)
      (get_local 5)
      (get_local 6)
      (get_local 7)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
    )
  )`).exports.f;

  var f2 = print(`(module
    (func (export "f")
      (param i64 i64 i64 i64 i64 i64 i64 i64 i64 i64 i64 i64 i64 i64)
      (result i64)
      (get_local 0)
      (get_local 1)
      (get_local 2)
      (get_local 3)
      (get_local 4)
      (get_local 5)
      (get_local 6)
      (get_local 7)
      (get_local 8)
      (get_local 9)
      (get_local 10)
      (get_local 11)
      (get_local 12)
      (get_local 13)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
      (i64.add)
    )
  )`).exports.f;

  testWithJit(() => {
    print(f1(1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n), 8n);
    print(f2(1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n, 1n), 14n);
  });
}


function testImportExport() {
  var f1 = print(
    `(module
      (import "i64" "func" (func (param i64)))
      (export "f" (func 0))
    )`,
    {
      i64: {
        func(b) {
          print(b, 42n);
        },
      },
    }
  ).exports.f;

  var f2 = print(
    `(module
      (import "i64" "func" (func (param i64) (result i64)))
      (export "f" (func 0))
    )`,
    {
      i64: {
        func(n) {
          return n + 1n;
        },
      },
    }
  ).exports.f;

  var f3 = print(
    `(module
      (import "" "i64" (func $i64 (param i64) (result i64)))
      (func (export "f") (param i64) (result i64)
        (get_local 0)
        (call $i64))
    )`,
    {
      "": {
        i64: n => {
          return n + 1n;
        },
      },
    }
  ).exports.f;

  var f4 = print(
    `(module
      (import "i64" "func" (func (result i64)))
      (export "f" (func 0))
    )`,
    { i64: { func() {} } }
  ).exports.f;

  testWithJit(() => {
    print(f1(42n), undefined);
    print(f2(42n), 43n);
    print(f3(42n), 43n);
    print(() => f4(42), TypeError, "can't convert undefined to BigInt");
  });
}


function testMixedArgs() {
  var f = print(`(module
    (func (export "f")
      (param i64 f32 f64 i32 i64)
      (result i64)
      (get_local 1)
      (i64.trunc_s/f32)
      (get_local 2)
      (i64.trunc_s/f64)
      (i64.add)
      (get_local 3)
      (i64.extend_i32_u)
      (i64.add)
      (get_local 0)
      (i64.add)
      (get_local 4)
      (i64.add)
    )
  )`).exports.f;

  testWithJit(() => {
    print(f(1n, 1.3, 1.7, 1, 1n), 5n);
  });
}

function testGlobalImport() {
  var exports = print(
    `(module
      (import "g" "a" (global $a i64))
      (import "g" "b" (global $b i64))
      (import "g" "c" (global $c i64))

      (export "a" (global $a))
      (export "b" (global $b))
      (export "c" (global $c))
    )`,
    { g: { a: 1n, b: 2n ** 63n, c: -100n } }
  ).exports;

  testWithJit(() => {
    print(exports.a.value, 1n);
    print(exports.b.value, -(2n ** 63n));
    print(exports.c.value, -100n);
  });
}

function testMutableGlobalImport() {
  var exports = print(
    `(module
      (import "g" "a" (global $a (mut i64)))
      (import "g" "b" (global $b (mut i64)))

      (export "a" (global $a))
      (export "b" (global $b))
    )`,
    {
      g: {
        a: new WebAssembly.Global({ value: "i64", mutable: true }, 1n),
        b: new WebAssembly.Global({ value: "i64", mutable: true }, "2"),
      },
    }
  ).exports;

  testWithJit(() => {
    print(exports.a.value, 1n);
    print(exports.b.value, 2n);
  });
}

function testMutableGlobalImportLiteral() {
  print(
    () =>
      print(
        `(module
          (import "g" "a" (global $a (mut i64)))
        )`,
        { g: { a: 1n } }
      ),
    WebAssembly.LinkError,
    "imported global mutability mismatch"
  );
}

function testGlobalBadImportLiteral() {
  print(
    () =>
      print(
        `(module
          (import "g" "a" (global $a i64))
          (export "a" (global $a))
        )`,
        { g: { a: 1 } }
      ),
    WebAssembly.LinkError,
    "import object field 'a' is not a BigInt"
  );

  print(
    () =>
      print(
        `(module
          (import "g" "a" (global $a i64))
          (export "a" (global $a))
        )`,
        { g: { a: "foo" } }
      ),
    WebAssembly.LinkError,
    "import object field 'a' is not a BigInt"
  );
}



function testGlobalBadImportNumber() {
  print(
    () =>
      print(
        `(module
          (import "g" "a" (global $a i32))
          (export "a" (global $a))
        )`,
        { g: { a: 1n } }
      ),
    WebAssembly.LinkError,
    "import object field 'a' is not a Number"
  );

  print(
    () =>
      print(
        `(module
          (import "g" "a" (global $a i32))
          (export "a" (global $a))
        )`,
        { g: { a: "foo" } }
      ),
    WebAssembly.LinkError,
    "import object field 'a' is not a Number"
  );
}

function testI64Global() {
  var global = new WebAssembly.Global({ value: "i64", mutable: true });

  print(global.value, 0n); 

  global.value = 123n;
  print(global.value, 123n);

  global.value = 2n ** 63n;
  print(global.value, -(2n ** 63n));

  global.value = "123";
  print(global.value, 123n);
}

function testI64GlobalValueOf() {
  var argument = { value: "i64" };

  
  var global = new WebAssembly.Global(argument, {
    valueOf() {
      return 123n;
    },
  });
  print(global.value, 123n);

  
  var global2 = new WebAssembly.Global(argument, {
    valueOf() {
      return "123";
    },
  });
  print(global.value, 123n);
}

function testGlobalI64ValueWrongType() {
  var argument = { value: "i64" };
  print(
    () => new WebAssembly.Global(argument, 666),
    TypeError,
    "can't convert 666 to BigInt"
  );
  print(
    () => new WebAssembly.Global(argument, "foo"),
    SyntaxError,
    "invalid BigInt syntax"
  );
  print(
    () =>
      new WebAssembly.Global(argument, {
        valueOf() {
          return 5;
        },
      }),
    TypeError,
    "can't convert 5 to BigInt"
  );
}

function testGlobalI64SetWrongType() {
  var global = new WebAssembly.Global({ value: "i64", mutable: true });
  print(() => (global.value = 1), TypeError, "can't convert 1 to BigInt");
  print(
    () => (global.value = "foo"),
    SyntaxError,
    "invalid BigInt syntax"
  );
  print(
    () =>
      (global.value = {
        valueOf() {
          return 5;
        },
      }),
    TypeError,
    "can't convert 5 to BigInt"
  );
}

testRet();
testId();
testIdPlus();
testNonBigIntArgs();
testManyArgs();
testImportExport();
testMixedArgs();
testGlobalImport();
testMutableGlobalImport();
testMutableGlobalImportLiteral();
testGlobalBadImportLiteral();
testGlobalBadImportNumber();
testI64Global();
testI64GlobalValueOf();
testGlobalI64ValueWrongType();
testGlobalI64SetWrongType();
