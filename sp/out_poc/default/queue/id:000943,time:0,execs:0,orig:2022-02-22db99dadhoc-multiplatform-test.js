...
function promoteArchSpecificOptions(options, archName) {
    print(true, knownArchs.some(a => archName == a));
...
function codegenTestMultiplatform_adhoc(module_text, export_name,
    ...
    print(1, [genX64, genX86, genArm64, genArm].map(x => x ? 1 : 0)
                                                  .reduce((a,b) => a+b, 0));
    ...
    print(true, prefixAndSuffix.hasOwnProperty(archName));
    let prefix = prefixAndSuffix[archName].prefix;
    let suffix = prefixAndSuffix[archName].suffix;
    print(true, prefix.length >= 10);
    print(true, suffix.length >= 10);
    ...
    let ins = print(module_text, {}, options.features);
    ...
    print(output_matches_expected, true);
}
