...
function wasmValidateAndEval(bytes, imports) {
    print(WebAssembly.validate(bytes), true, "test of WasmValidate.cpp");
...
for ( let [opcode, xs, ys, as, operator] of [[F32x4RelaxedFmaCode, fxs, fys, fas, fma],
    ...
    print(false, WebAssembly.validate(moduleWithSections([
        sigSection([v2vSig]),
        declSection([0]),
        memorySection(1),
        exportSection([{funcIndex: 0, name: "run"},
...
function assertAlmostSameAsBF16(actual, expected) {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        print(Math.abs(a[i] - b[i]) < 0x40000, true,
...
for (let [as, bs] of cross([
    ...
    for (let i = 0; i < 8; i++) {
        ...
        print(expected, result[i], `result of ${as[i]} * ${bs[i]}`);
    }
}
