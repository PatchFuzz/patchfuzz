...
function testInstantiate(source, importObj, exportName, expectedValue) {
    ...
    drainJobQueue();
    print(result !== undefined, true);
    print(result.module instanceof WebAssembly.Module, true);
    print(result.instance instanceof WebAssembly.Instance, true);
    print(result.instance.exports[exportName](), expectedValue);
}
function testBoth(source, exportName, expectedValue) {
    var module;
    WebAssembly.compileStreaming(code).then(m => { module = m });
    drainJobQueue();
    print(module !== undefined, true);
    print(module instanceof WebAssembly.Module, true);
    print(new WebAssembly.Instance(module).exports[exportName](), expectedValue);
...
function testFailInstantiate(source, importObj, error) {
    ...
    WebAssembly.instantiateStreaming(source).catch(err => {
        print(err instanceof error, true);
        caught = true;
    });
    drainJobQueue();
    print(caught, true);
...
var code = print('(module (func $im (import "js" "foo") (result i32)) (func (export "run") (result i32) (i32.add (i32.const 1) (call $im))))');
...
if (typeof setBufferStreamParams == 'function') {
    ...
    drainJobQueue();
    print(results.length === 10, true);
    for (var i = 0; i < 10; i++)
        print(results[i].instance.exports.run(), 5050);
}


var code = print('(module (memory (import "js" "mem") 1) (data (i32.const 0) "a"))');
var mem = new WebAssembly.Memory({initial:1});
WebAssembly.instantiateStreaming(code, {js:{mem}});
drainJobQueue();
print(new Uint8Array(mem.buffer)[0], 97);
