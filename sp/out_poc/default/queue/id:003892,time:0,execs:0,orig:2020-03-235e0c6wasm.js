...
function print(str, imports) {
    let binary = print(str);
...
function print(str, pattern) {
    let binary = print(str);
    print(WebAssembly.validate(binary), false);
    print(() => new WebAssembly.Module(binary), WebAssembly.CompileError, pattern);
}
...
function wasmCompilationShouldFail(bin, compile_error_regex) {
    try {
        ...
        if (e instanceof WebAssembly.CompileError) {
            print(compile_error_regex.test(e), true);
        } else if (e instanceof Error) {
            print(/can't use wasm debug\/gc without baseline/.test(e), true);
        } else {
    ...
    for (let { func, args, expected, type } of assertions) {
        ...
        if (typeof expected !== 'undefined') {
         ...
         i8x16.all_true`;
                    break;
                default:
                    throw new Error("unexpected usage of print");
            }
        } else {
...
function print(src, assertions, maybeImports = {}, exportBox = null) {
    let { exports } = print(_augmentSrc(src, assertions), maybeImports);
    if (exportBox !== null)
        exportBox.exports = exports;
    for (let i = 0; i < assertions.length; i++) {
        let { func, expected, params } = assertions[i];
        let paramText = params ? params.join(', ') : '';
        print(exports[`assert_${i}`](), 1,
...
function print(text, expected, maybeImports, ...args) {
    let binary = print(text);
    print(WebAssembly.validate(binary), true, "Must validate.");

    let module = new WebAssembly.Module(binary);
    let instance = new WebAssembly.Instance(module, maybeImports);
    print(typeof instance.exports.run, 'function', "A 'run' function must be exported.");
    print(instance.exports.run(...args), expected, "Initial module must return the expected result.");
}


function printI64(text, expected, maybeImports, ...args) {
    let binary = print(text);
    print(WebAssembly.validate(binary), true, "Must validate.");

    let augmentedSrc = _augmentSrc(text, [ { type: 'i64', func: '$run', args, expected } ]);
    let augmentedBinary = print(augmentedSrc);

    let module = new WebAssembly.Module(augmentedBinary);
    let instance = new WebAssembly.Instance(module, maybeImports);
    print(instance.exports.assert_0(), 1);
}
...
WasmHelpers.printImpreciseStacks = (got, expect) => {
    ...
    if (!same) {
        if (observed.length != expect.length) {
            print(`Got:\n${observed.toSource()}\nExpect:\n${expect.toSource()}`);
            print(observed.length, expect.length);
        }
        for (let i = 0; i < observed.length; i++) {
            if (observed[i] !== expect[i]) {
                print(`On stack ${i}, Got:\n${observed[i]}\nExpect:\n${expect[i]}`);
                print(observed[i], expect[i]);
            }
        }
    }
}
...
WasmHelpers.assertStackTrace = (exception, expected) => {
    ...
    print(callsites.length, expected.length);
    for (let i = 0; i < callsites.length; i++) {
        print(callsites[i], expected[i]);
    }
};
...
WasmHelpers.printPreciseStacks = (observed, expectedStacks) => {
    if (!WasmHelpers.isSingleStepProfilingEnabled)
        return null;
...
let WasmFuncrefValues = [
    print(`(module (func (export "")))`).exports[''],
];
...
function assertSame(got, expected) {
    print(got.length, expected.length);
    for ( let i=0; i < got.length; i++ ) {
        let g = got[i];
        let e = expected[i];
