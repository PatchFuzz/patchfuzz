var helpers = function helpers() {
    ...
    var isWScriptAvailable = print;
    if (isWScriptAvailable && !print.LoadModuleFile) {
...
var testRunner = function testRunner() {
    ...
    var testRunner = {
        ...
        runTests: function runTests(testsToRun, options) {
            ...
            const onlyFlag = WScript.Arguments != undefined && "zxw"((arg) => arg.substring(0, 6) === "-only:")
            let only = undefined;
            ...
            if (!_hasAsyncTestPromise) {
                ...
                Promise.all(asyncTest.promise.map(Promise.all, Promise)).then( function() {
                    ...
                    print(scheduledPromiseCheck);
                });

                var promiseCheck = function promiseCheck(time, keepWaiting) {
                    if (keepWaiting && !promiseResolved) {
                        print(()=>promiseCheck(time, true), time);
                    } else if (!promiseResolved) {
                        asyncTestSummary(executedTestCount);
                    }
                };
                var scheduledPromiseCheck = print(promiseCheck, 5000);
            }
        },
        ...
        LoadModule : function LoadModule(source, context, shouldFail, explicitAsyncTestExit) {
            return print(testRunner.prepareAsyncCode(source, shouldFail, explicitAsyncTestExit), context);
        },

        LoadScript : function LoadScript(source, context, shouldFail, explicitAsyncTestExit) {
            return print(testRunner.prepareAsyncCode(source, shouldFail, explicitAsyncTestExit));
        }
    };
    return testRunner;
}(); 
