...
function test(module, {exports} = {}) {
  if (module) {
    ...
    print("exports");
    print(objectToString(WebAssembly.Module.exports(module)));
    print("imports");
    print(objectToString(WebAssembly.Module.imports(module)));
  }
  if (exports) {
    print("Testing instance");
    print(`f1: ${exports.f1()}`);
    print(`fn: ${exports.fn()}`);
    print(`fn2: ${exports.fn2()}`);
    print(`g1: ${exports.g1}`);
    print(`g2: ${exports.g2}`);
    print(`g3: ${exports.g3}`);
  }
...
async function testInvalidCases(tests) {
  ...
  for (const testCase of tests) {
    ...
    try {
      await testCase();
      print(`Test ${i++} failed. Should have thrown error`);
    } catch (e) {
      print(`Test ${i++} passed. Expected Error: ${e}`);
    }
  }
}
...
async function testValidate() {
  ...
  print(`WebAssembly.validate(new ArrayBuffer()) = ${WebAssembly.validate(emptyBuffer)}`);
  print(`WebAssembly.validate(new Uint8Array(emptyBuffer)) = ${WebAssembly.validate(new Uint8Array(emptyBuffer))}`);
  print(`api.wasm valid: ${WebAssembly.validate(buf)}`);
}
...
async function testModuleCustomSection(baseModule) {
  ...
  if (!Array.isArray(baseSections) || baseSections.length !== 0) {
    print("Invalid result for WebAssembly.Module.customSections");
    print(baseSections);
  }
  ...
  function compare(module, customSectionName, expected) {
    ...
    if (sections.length !== expected.length) {
      print(`Invalid length. Got ${sections.length}, expected ${expected.length}. ${(new Error()).stack}`);
      ++failed;
      return;
    }
    let asExpected = true;
    for (let iSection = 0; iSection < sections.length; ++iSection) {
      ...
      if (resStr !== expectedStr) {
        asExpected = false;
        print(`Invalid buffer result. Got ${resStr}, expected ${expectedStr}. ${(new Error()).stack}`);
      }
    }
    passed += asExpected;
    failed += !asExpected;
...
async function testInstanceConstructor(module) {
  ...
  await testInvalidCases([
    ...
    () => new WebAssembly.Instance(module, overrideImports({fn2: instance.exports.fn })),
...
async function testMemoryApi(baseModule) {
  ...
  view[0] = 45;
  print(`heap32[0] = ${load(0)}`);

  function testOutOfBounds(index) {
    try {
      load(index);
      print("Should have trap with out of bounds error");
    } catch (e) {
      if (e instanceof WebAssembly.RuntimeError) {
        print(`Correctly trap on heap access at ${index}`);
      } else {
  ...
  print(`memory.buffer.byteLength = ${memory.buffer.byteLength} == ${pageSize}`);
  {
    view[pageSize / 4 - 1] = 0x12345678;
    print(`view32[${pageSize / 4 - 1}] = ${toHex(view[pageSize / 4 - 1])}`);
    print(`view32[${pageSize / 4}] = ${toHex(view[pageSize / 4])}`);
    print(`heap[${pageSize - 4}] = ${toHex(load(pageSize - 4))}`);
    testOutOfBounds(pageSize - 3);
    testOutOfBounds(pageSize - 2);
    testOutOfBounds(pageSize - 1);
    testOutOfBounds(pageSize);
  }
  print("grow by 1 page");
  memory.grow(1);
  print(`memory.buffer.byteLength = ${memory.buffer.byteLength} == 2 * ${pageSize}`);
  view = new Int32Array(memory.buffer); 
  {
    view[2 * pageSize / 4 - 1] = 0x87654321;
    print(`view32[${2 * pageSize / 4 - 1}] = ${toHex(view[2 * pageSize / 4 - 1])}`);
    print(`view32[${2 * pageSize / 4}] = ${toHex(view[2 * pageSize / 4])}`);
    print(`heap[${pageSize - 4}] = ${toHex(load(pageSize - 4))}`);
    print(`heap[${2 * pageSize - 4}] = ${toHex(load(2 * pageSize - 4))}`);
  ...
  try {
    memory.grow(1);
    print("Should have trap when growing past maximum");
  } catch (e) {
    if (e instanceof RangeError) {
      print("Correctly trap when growing past maximum");
    } else {
      print(`Unexpected Error: ${e}`);
    }
  }
...
async function testTableApi(baseModule) {
  ...
  print(`Current length: ${table.length}`);
  table.length = 456;
  print(`Length after attempt to modify : ${table.length}`);
  print(`Is element in table the same as the one exported: ${table.get(1) === fn}`);
  print(`Unset element should be null: ${table.get(0)}`);
  table.set(0, myFn);
  print(`call_i32(0): ${call_i32(0)}`);
  print(`call_i32(1): ${call_i32(1)}`);
  print(`call_f32(2): ${call_f32(2)}`);
  table.set(0, fn);
  print(`call_i32(0): ${call_i32(0)}`);
  table.set(29, myFn);
  table.grow(1);
  table.set(30, myFn);
  print(`call_i32(29): ${call_i32(29)}`);
  print(`call_i32(30): ${call_i32(30)}`);

  const table2 = new WebAssembly.Table({element: "anyfunc", initial: 0});
  table.grow(0);
  try {table2.get(0); print("Failed. Unexpected successfull call to table2.get(0)");} catch (e) {}
  table.grow(1);
  table.set(0, myFn);
  print(`table2[0](): ${table.get(0)()}`);
}
