...
function test(fn, expectedRes, ...args) {
  ...
  if (isNaN(expectedRes)) {
    if (!isNaN(res)) {
      print(`Failed: ${fn}(${args.join(", ")}) expected NaN. Got ${res}`);
    }
  } else if (Object.is(expectedRes, -0)) {
    if (!Object.is(res, -0)) {
      print(`Failed: ${fn}(${args.join(", ")}) expected -0. Got ${res}`);
    }
  } else if (res !== Math.fround(expectedRes)) {
      print(`Failed: ${fn}(${args.join(", ")}) expected ${Math.fround(expectedRes)}. Got ${res}`);
  }
}
