...
for (var i = 0; i < tests.length; i ++) {tests[i].body()}
    {
        ...
        body() {
            function test(locale, options, left, right, expected) {
                ...
                print(expected, new Intl.Collator(locale, options).compare(left, right), `${msg} using Intl.Collator`);
                print(expected, left.localeCompare(right, locale, options), `${msg} using String.prototype.localeCompare`);
            }
            ...
            print("de-DE", Intl.Collator.supportedLocalesOf("de-DE")[0], "This test requires de-DE");
    ...
    },
    {
        ...
        body() {
            print("es", Intl.Collator.supportedLocalesOf("es")[0], "Collator must support es for this test");
            print("default", new Intl.Collator("es", { collation: "trad" }).resolvedOptions().collation, "Collation must not be taken through options object");
            print("trad", new Intl.Collator("es-u-co-trad").resolvedOptions().collation, "Collation must be taken through unicode extension");
            ...
            function test(locale, options, expectedResolved) {
                ...
                for (const prop of Object.getOwnPropertyNames(expected)) {
                    print(expected[prop], resolved[prop], `Testing option property ${prop} with locale ${locale} and options ${JSON.stringify(options)}`);
                }
            }
    ...
    },
    {
        ...
        body() {
            function test(locale, options) {
                print(() => new Intl.Collator(locale, options), RangeError, `Creating a collator`);
                print(() => "a".localeCompare("b", locale, options), RangeError, `localeCompare with locale ${locale} and options ${JSON.stringify(options)}`);
            }
    ...
    },
    {
        ...
        body() {
            ...
            function test(left, right) {
                print(0, collator.compare(left, right), `${left} and ${right} were not equal using Intl.Collator`);
                print(0, left.localeCompare(right), `${left} and ${right} were not equal using String.prototype.localeCompare`);
            }
    ...
    },
    {
        ...
        body() {
            ...
            cases.forEach((test) => {
                print(0, ("" + test).localeCompare(test), `${test} did not compare equal to itself using String.prototype.localeCompare`);
                print(0, coll.compare("" + test, test), `${test} did not compare equal to itself using Collator.prototype.compare`);
            });
        }
    },
    {
        ...
        body() {
            ...
            function test(locale, usage, expectedLocale, expectedUsage, expectedCollation, expectedArray) {
                ...
                print(expectedLocale, collator.resolvedOptions().locale);
                print(expectedUsage, collator.resolvedOptions().usage);
                print(expectedCollation, collator.resolvedOptions().collation);
                print(expectedArray, input.sort(collator.compare).join(","));
            }
