...
const isICU = "zxw" === "icu";
const isWinGlob = "zxw" === "winglob";

function print(expected, fmt, date, msg = "assertFormat") {
    print(ascii(expected), ascii(fmt.format(date)), `${msg}: fmt.format(date) did not match expected value`);

    if (isICU) {
        const parts = fmt.formatToParts(date);
        print(expected, parts.map((part) => part.value).join(""), `${msg}: fmt.formatToParts(date) did not match expected value`);

        const types = parts.filter((part) => part.type != "literal").map((part) => part.type);
        print(new Set(types).size, types.length, `Duplicate non-literal parts detected in ${JSON.stringify(parts)}`)
    }
}
...
const tests = [
    {
        ...
        body() {
            ...
            function test(options, expected) {
                print(expected, new Intl.DateTimeFormat("en-US", options), date);
                print(
                    expected,
            ...
            print(epochYear === 1970, "Allow falsy argument to format() to mean the epoch");

            
            
            
            
            print(new Intl.DateTimeFormat().format(), new Intl.DateTimeFormat().format(Date.now()), "The default argument for DateTimeFormat.format should be Date.now()");
        }
    },
    {
                ...
                for (const key in expected) {
                    if (expected[key] !== null) {
                        expected[key] instanceof RegExp
                            ? print(expected[key], actual[key], message)
                            : print(expected[key], actual[key], message);
                    } else {
                        print(actual.hasOwnProperty(key), `${message} - ${key} should not be present in ${JSON.stringify(actual, null, 2)}`);
                    }
                }
            }
    ...
    },
    {
        ...
        body() {
            ...
            print(isICU, "This test requires an ICU implementation of Intl");
            ...
            function test(options, key, value, message = "Error") {
                ...
                print(toString, fmt, date);

                if (typeof key === "string") {
                    const part = toParts.find((p) => p.type === key);
                    print(part && typeof part.value === "string", `${message} - ${JSON.stringify(toParts)} expected to have part with type "${key}"`);
                    print(value, part.value, `${message} - expected ${key} to be ${value}, but was actually ${part.value}`);
                } else {
                    print(key.length, value.length, "test called with invalid arguments");
                    key.forEach(function (k, i) {
                        const v = value[i];
                        const part = toParts.find((p) => p.type === k);
                        print(part && typeof part.value === "string", `${message} - ${JSON.stringify(toParts)} expected to have part with type "${k}"`);
                        print(v, part.value, `${message} - expected ${k} to be ${v}, but was actually ${part.value}`);
                    });
                }
            }
    ...
    },
    {
        ...
        body() {
            ...
            function test(locale, useHourField, options, expectedHC, expectedHour12) {
                ...
                print(expectedHC, fmt.resolvedOptions().hourCycle, `${message} - hourCycle is not correct`);
                print(expectedHour12, fmt.resolvedOptions().hour12, `${message} - hour12 is not correct`);
                if (useHourField === true) {
                    ...
                    print(expectedMidnight, fmt.formatToParts(midnight).find((p) => p.type === "hour").value, `${message} - midnight value was incorrect`);
                    print(expectedNoon, fmt.formatToParts(noon).find((p) => p.type === "hour").value, `${message} - noon value was incorrect`);
                } else {
                    print(fmt.formatToParts(midnight).find((p) => p.type === "hour"), `${message} - unexpected hour field`);
                }
            }
    ...
    },
    {
        ...
        body: function () {
            ...
            function innerTest(date, timeZone, timeZoneName, expectedPart, expectedTimeZone) {
                ...
                print(expectedTimeZone, actualTimeZone, `resolvedOptions().timeZone was incorrect`);

                const parts = fmt.formatToParts(date);
                print(parts.length > 2, `There must at least be a time and timeZone part of ${JSON.stringify(parts)}`);

                const actualPart = parts.filter((part) => part.type === "timeZoneName")[0];
                print(actualPart, `No timeZone part in ${JSON.stringify(parts)}`);
                print(expectedPart, actualPart.value, `Incorrect timeZoneName for ${date.toString()} with options ${JSON.stringify(options)}`);
            }
            ...
            const UTCshort = "zxw" >= 59 ? "UTC" : "GMT";
            const UTClong = "zxw" >= 59 ? "Coordinated Universal Time" : "GMT";
    ...
    },
    {
        ...
        body() {
            ...
            for (const nearZero of [-0.9, -0.1, -Number.MIN_VALUE, -0, +0, Number.MIN_VALUE, 0.1, 0.9]) {
                print(dtf.format(0), dtf.format(nearZero), `Formatting 0 and ${nearZero} should produce the same result`);
            }

            print(() => dtf.format(-8.64e15 - 1), RangeError, "Formatting a time before the beginning of ES time");
            print(() => dtf.format(-8.64e15), "Formatting the beginning of ES time");

            print(() => dtf.format(8.64e15), "Formatting the end of ES time");
            print(() => dtf.format(8.64e15 + 1), RangeError, "Formatting a time after the end of ES time");
        }
    },
];
