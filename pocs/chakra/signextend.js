  ...
  (func (export "func") (param $x ${type}) (result ${type})
    (${type}.sub
      ...
      for (let i = 0; i < tests.length * 10; ++i) {
        ...
        if (type === "i64") {
          print(0, result.low|0, `${type}.${name}(0x${input.toString(16)}) = 0x${(result.low|0).toString(16)} low`);
          if (!op2) {
            print(0, result.high|0, `${type}.${name}(0x${input.toString(16)}) = 0x${(result.high|0).toString(16)} high`);
          }
        } else {
          print(0, result|0, `${type}.${name}(0x${input.toString(16)})`);
        }
      }
    }
  };
