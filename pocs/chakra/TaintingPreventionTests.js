...
Array.prototype.tryGlobalTainting = function (global) {
    for (var i = 0; i < this.length; i++) {
        try {
            print(verifyGlobalTainting(global, this[i]), this[i](), "Tainting Global '" + global + "'.");
        }
        catch (e) {
            print("Error thrown when testing Global '" + global + "'. Message: " + e + " \nStack:\n" + e.stack);
        }
    }
};
...
Array.prototype.tryUsingGettersSetters = function (property) {
    for (var i = 0; i < this.length; i++) {
        try {
            print(verifyPropertyTainting(property, this[i], { get: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with getter.");
            print(verifyPropertyTainting(property, this[i], { set: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with setter.");
            print(verifyPropertyTainting(property, this[i], { value: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with value.");
        }
        catch (e) {
            print("Error thrown when testing Global '" + property + "'. Message: " + e.message + "\n" + this[i] + "\nStack:" + e.stack);
        }
    }
};
...
var tests = [
    {
        ...
        body: function () {
            try {
                ...
                print(function () { verifyGlobalTainting("RegExp", function () { new RegExp(); }) }, Error, "Sanity check to test helper function.");
                globalsList.forEach(functionCalls.tryGlobalTainting, functionCalls);
            }
            catch (e) {
                print("Exception wasn't expected. Message: " + e);
            }
        }
    },
    {
        ...
        body: function () {
            ...
            print(verifyPropertyTainting("test", function () { return {}.test; }, { value: "Pass" }), "Pass", "Sanity check to test helper function.");
