...
var tests = [{
		...
		body : function () {
			print(() => Atomics(), TypeError, "Atomics cannot be called", "Function expected");
			print(() => new Atomics(), TypeError, "Atomics cannot be called with new", "Object doesn't support this action");
            for (var {op, length} of allAtomicsFunctionsList) {
                print(typeof op, "function", "Atomics has " + op.name + " as a function");
                print(op.length, length, "Validating " + op.name + ".length");
                print(() => new op(), TypeError, op.name +" cannot be called with new", "Function is not a constructor");
            }
		}
	}, {
		...
		body : function () {
            ...
            for (var {op, fullname} of allAtomicsFunctionsList) {
                print(() => op(), RangeError, op.name +" requires parameter", fullname+": function called with too few arguments");
            }
            for (var {op, fullname, onlyInt32} of [...atomicsFunctionsList, ...atomicsFunctionsList2]) {
                [undefined, null, 1, {}, [], new Array()].forEach(function(o) {
                    print(() => op(o, 0, 0, 0), TypeError, op.name +" is not allowed on this object",
                                        "Atomics function called with invalid typed array object");
                });
                    if (onlyInt32)
                    {
                        print(() => op(new Int8Array(1), 0, 0, 0), TypeError, op.name +" is not allowed on Int8Array of ArrayBuffer",
                                        "The operation is not supported on this typed array type");
                        for (var j of nonInt32Views) {
                            print(() => op(new Int8Array(1), 0, 0, 0), TypeError, op.name +" is not allowed",
                                            "The operation is not supported on this typed array type");
                        }
                    }
                    else
                    ...
                    print(() => op(new Float32Array(1), 0, 0, 0), TypeError, op.name +" is not allowed on Float32Array of ArrayBuffer",
                                        "The operation is not supported on this typed array type");
                    print(() => op(new Float32Array(makeSharedArrayBuffer(8)), 0, 0, 0), TypeError, op.name +" is not allowed on Float32Array",
                                        "The operation is not supported on this typed array type");
                    print(() => op(new Float64Array(makeSharedArrayBuffer(8)), 0, 0, 0), TypeError, op.name +" is not allowed on Float64Array",
                                        "The operation is not supported on this typed array type");
            }
		}
	},
            ...
            atomicsFunctionsList.forEach(function(atomicFunction) {
                ...
                IntViews.forEach(function(item) {
                    [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                        ...
                        print(() => atomicFunction.op(view), RangeError, "Calling "+atomicFunction.fullname+" with 1 param only is not valid", atomicFunction.fullname+": function called with too few arguments");
                        print(() => atomicFunction.op(view, 0), RangeError, "Calling "+atomicFunction.fullname+" with 2 params only is not valid", atomicFunction.fullname+": function called with too few arguments");
                        [undefined, 1.1, "hi", NaN, {}].forEach(function (index) {
                            atomicFunction.op(view, index, 1);
                        });

                        [-1, Infinity, -Infinity].forEach(function (index) {
                           print(() => atomicFunction.op(view, index, 1), RangeError, "Only positive interger allowed, not " + index, "Access index is out of range");
                        });

                        print(() => atomicFunction.op(view, elements, 1), RangeError, "index is out of bound " + elements, "Access index is out of range");
                        print(() => atomicFunction.op(view, length, 1), RangeError, "index is out of bound " + length, "Access index is out of range");
                        ...
                        print(() => atomicFunction.op(view, elements + 1, 1), RangeError, "Changing 'length' property does not affect the index", "Access index is out of range");
		...
		body : function () {
            IntViews.forEach(function(item) {
                [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                    ...
                    print(() => Atomics.compareExchange(view), RangeError, "Calling Atomics.compareExchange with 1 param only is not valid", "Atomics.compareExchange: function called with too few arguments");
                    print(() => Atomics.compareExchange(view, 0), RangeError, "Calling Atomics.compareExchange with 2 params only is not valid", "Atomics.compareExchange: function called with too few arguments");
                    print(() => Atomics.compareExchange(view, 0, 0), RangeError, "Calling Atomics.compareExchange with 3 params only is not valid", "Atomics.compareExchange: function called with too few arguments");
                    [undefined, 1.1, "hi", NaN, {}].forEach(function (index) {
                        Atomics.compareExchange(view, index, 0, 0);
                    });

                    [-1, Infinity, -Infinity].forEach(function (index) {
                        print(() => Atomics.compareExchange(view, index, 0, 0), RangeError, "Only positive interger allowed not, " + index, "Access index is out of range");
                    });

                    print(() => Atomics.compareExchange(view, elements, 0, 0), RangeError, "index is out of bound " + elements, "Access index is out of range");
                    print(() => Atomics.compareExchange(view, length, 0, 0), RangeError, "index is out of bound " + length, "Access index is out of range");
                });
            });
		}
	},
		...
		body : function () {
            IntViews.forEach(function(item) {
                [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                    ...
                    print(() => Atomics.load(view), RangeError, "Calling Atomics.load with 1 param only is not valid", "Atomics.load: function called with too few arguments");
                    [undefined, 1.1, "hi", NaN, {}].forEach(function (index) {
                        Atomics.load(view, index);
                    });

                    [-1, Infinity, -Infinity].forEach(function (index) {
                        print(() => Atomics.load(view, index), RangeError, "Only positive interger allowed, not " + index, "Access index is out of range");
                    });

                    print(() => Atomics.load(view, elements), RangeError, "index is out of bound " + elements, "Access index is out of range");
                    print(() => Atomics.load(view, length), RangeError, "index is out of bound " + length, "Access index is out of range");
                });
            });
		}
	},
    {
		...
		body : function () {
            [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                ...
                print(() => Atomics.wait(view), RangeError, "Calling Atomics.wait with 1 param only is not valid", "Atomics.wait: function called with too few arguments");
                print(() => Atomics.wait(view, 0), RangeError, "Calling Atomics.wait with 1 param only is not valid", "Atomics.wait: function called with too few arguments");
                [undefined, 1.1, "hi", NaN, {}].forEach(function (index) {
                    Atomics.wait(view, index, 1);
                });

                [-1, Infinity, -Infinity].forEach(function (index) {
                    print(() => Atomics.wait(view, index, 0), RangeError, "Only positive interger allowed, not " + index, "Access index is out of range");
                });

                print(() => Atomics.wait(view, elements, 0), RangeError, "index is out of bound " + elements, "Access index is out of range");
                print(() => Atomics.wait(view, length, 0), RangeError, "index is out of bound " + length, "Access index is out of range");
            });
		}
	},
    {
		...
		body : function () {
            [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                ...
                print(() => Atomics.notify(view), RangeError, "Calling Atomics.notify with 1 param only is not valid", "Atomics.notify: function called with too few arguments");
                [undefined, 1.1, "hi", NaN, {}].forEach(function (index) {
                    Atomics.notify(view, index, 1);
                });

                [-1, Infinity, -Infinity].forEach(function (index) {
                    print(() => Atomics.notify(view, index, 1), RangeError, "Only positive interger allowed, not " + index, "Access index is out of range");
                });

                print(() => Atomics.notify(view, elements, 1), RangeError, "index is out of bound " + elements, "Access index is out of range");
                print(() => Atomics.notify(view, length, 1), RangeError, "index is out of bound " + length, "Access index is out of range");
            });
		}
	},
    {
            ...
            function ValidateAtomicOpFunctionality(ctor, atomicsOp, data) {
                [[0, 8, 8], [6, 2, 8]].forEach(function([offset, length, elements]) {
                    ...
                    for (var {initValue, opValue, expectedReturn, expectedValue} of data) {
                        ...
                        print(ret, expectedReturn, "return value validation " + ctor.name + " " + atomicsOp.name);
                        print(view[1], expectedValue, "final value validation " + ctor.name + " " + atomicsOp.name);
                        print(Atomics.load(view, 1), expectedValue, "final value validation " + ctor.name + " " + atomicsOp.name);
                        print(view[0], 0, "view[0] should not be affected");
                    }
                });
            }
		...
		body : function () {
            [[0, 4, 4], [3, 2, 8]].forEach(function([offset, length, elements]) {
                IntViews.forEach(function ({ctor}) {
                    ...
                    print(ret, 20, "value validation " + ctor.name);
                    print(view[0], 0, "view[0] should not be affected");
                });
            });
		}
		...
		body : function () {
            IntViews.forEach(function({ctor}) {
                [[0, 8, 8], [6, 2, 8]].forEach(function([offset, length, elements]) {
                    ...
                    print(ret, 10, "return value validation " + ctor.name);
                    print(view[1], 20, "value validation " + ctor.name);

                    var ret = Atomics.compareExchange(view, 1, 5, 30);
                    print(ret, 20, "compared failed -  return value validation " + ctor.name);
                    print(view[1], 20, "compared failed - value validation " + ctor.name);
                });
            });
		}
	},
		...
		body : function () {
            ...
            IntViews.forEach(function({ctor}) {
                [undefined, NaN, null, Number.POSITIVE_INFINITY, true, "1", "blah", 1.1, valueOf, toString].forEach(function(value) {
                    ...
                    print(ret, 10, "return value validation " + ctor.name);
                    print(view[1], 10 + num, "final value validation " + ctor.name);
                    ...
                    print(ret, 10, "return value validation " + ctor.name);
                    print(view[1], num, "final value validation " + ctor.name);
                    ...
                    {
                        print(ret, num, "return value validation " + ctor.name);
                    }
                    print(view[1], num, "final value validation " + ctor.name);
	...
	},
    {
		...
		body : function () {
            ...
            print(view[1], 2, "Changing view's length does not affect the 'add'");

            view[1] = 0;
            Atomics.add(view, 1, lengthChange1);
            print(view[1], 2, "Changing view's length by defineProperty does not affect the 'add'");

            view[1] = 0;
            Atomics.add(view, 1, bufferChange);
            print(view[1], 2, "Changing the buffer does not affect the 'add'");
		}
	},
    {
		...
		body : function () {
            ...
            print(sab instanceof MySAB, "object is instance of subclass of SharedArrayBuffer");
            print(sab instanceof SharedArrayBuffer, "object is instance of SharedArrayBuffer");
            var view = new Int8Array(sab, 0, 8);
            view[1] = 10;
            var ret = Atomics.add(view, 1, 20);
            print(ret, 10, "return value validation ");
            print(Atomics.load(view, 1), 30, "final value validation");
		}
	},
    {
		...
		body : function () {
            ...
            print(() => Atomics.wait(view, 0, 0, 0), "Atomics.wait is allowed on the index 0, where the view's length is 2");
            print(() => Atomics.wait(view, "0", 0, 0), "ToNumber : Atomics.wait is allowed on the index 0, where the view's length is 2");
            print(() => Atomics.wait(view, 2, 0, 0), RangeError, "Index is greater than the view's length", "Access index is out of range");
            print(() => Atomics.wait(view, -1, 0, 0), RangeError, "Negative index is not allowed", "Access index is out of range");
		}
	},
    {
		...
		body : function () {
            ...
            [2, "2", {valueOf : () => 2}].forEach(function(value) {
                ...
                print(ret, "not-equal", "value is not matching with 1 and 'wait' will return as 'not-equal'");
            });

            [10, "10", {valueOf : () => 10}].forEach(function(value) {
                view[0] = 10;
                var ret = Atomics.wait(view, 0, value, 5);
                print(ret, "timed-out", "value should match with 10 and 'wait' will wait till time-out");
            });

            view[0] = 10;
            var ret = Atomics.wait(view, 0, 10, Number.NEGATIVE_INFINITY);
            print(ret, "timed-out", "Negative infinity will be treated as 0 and so this will time-out");
		}
	},
    {
		...
		body : function () {
            [2**15, 2**20, 2**25].forEach(function(size) {
                IntViews.forEach(function(item) {
                    ...
                    print(Atomics.load(view, 0), 10);
                    print(Atomics.load(view, 2**10), 20);
                });
            });
		}
	},
