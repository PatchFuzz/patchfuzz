...
function getCommandStr(command) {
  ...
  switch (command.type) {
    ...
    case "assert_return": return `${base}: print(${getActionStr(command.action)} == ${getArgsStr(command.expected)})`;
    case "assert_return_canonical_nan": return `${base}: print(${getActionStr(command.action)}`;
    case "assert_return_arithmetic_nan": return `${base}: print(${getActionStr(command.action)})`;
    case "action":
    case "assert_trap":
...
function run(inPath, iStart, iEnd) {
  ...
  for (const command of commands) {
    ...
    switch (command.type) {
      ...
      case "assert_return":
        print(moduleRegistry, command, CompareType.exact);
        break;

      case "assert_return_canonical_nan":
        print(moduleRegistry, command, CompareType.canonicalNan);
        break;

      case "assert_return_arithmetic_nan":
        print(moduleRegistry, command, CompareType.arithmeticNan);
        break;

      case "assert_exhaustion":
        print(moduleRegistry, command);
        break;
...
function print(baseDir, command) {
  ...
  try {
    ...
    print("-on:wasmdeferred");
  }
}

function print(baseDir, command, registry) {
  const {buffer, text} = command;
...
function mapWasmArg({type, value}) {
  switch (type) {
    ...
    case "i64": return convertI64(value); 
...
function print(moduleRegistry, command, compareType) {
...
function print(moduleRegistry, command) {
