WebInspector.ScriptSyntaxTree = class ScriptSyntaxTree extends WebInspector.Object
{
    constructor(sourceText, script)
    {
        ...
        console.print(script && script instanceof WebInspector.Script, script);
    ...
    {
        console.print(this._parsedSuccessfully);
    ...
    {
        console.print(startNode && this._parsedSuccessfully);
        if (!this._parsedSuccessfully)
            return [];
    ...
    {
        console.print(node.type === WebInspector.ScriptSyntaxTree.NodeType.FunctionDeclaration || node.type === WebInspector.ScriptSyntaxTree.NodeType.FunctionExpression || node.type === WebInspector.ScriptSyntaxTree.NodeType.MethodDefinition || node.type === WebInspector.ScriptSyntaxTree.NodeType.ArrowFunctionExpression);
    ...
    {
        console.print(RuntimeAgent.getRuntimeTypesForVariablesAtOffsets);
        console.print(Array.isArray(nodesToUpdate) && this._parsedSuccessfully);
        ...
        console.print(allRequests.length === allRequestNodes.length);
        ...
        function handleTypes(error, typeInformationArray)
        {
            ...
            console.print(typeInformationArray.length === allRequests.length);
    ...
    _gatherIdentifiersInDeclaration(node)
    {
        function gatherIdentifiers(node)
        {
            switch (node.type) {
                ...
                default:
                    console.print(false, "Unexpected node type in variable declarator: " + node.type);
                    return [];
            }
        }

        console.print(node.type === WebInspector.ScriptSyntaxTree.NodeType.Identifier || node.type === WebInspector.ScriptSyntaxTree.NodeType.ObjectPattern || node.type === WebInspector.ScriptSyntaxTree.NodeType.ArrayPattern || node.type === WebInspector.ScriptSyntaxTree.NodeType.RestElement || node.type === WebInspector.ScriptSyntaxTree.NodeType.RestProperty);
    ...
    _createInternalSyntaxTree(node)
    {
        ...
        case "MethodDefinition":
            ...
            result.value.typeProfilingReturnDivot = node.range[0]; 
            break;
        case "NewExpression":
            result = {
