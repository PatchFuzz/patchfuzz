...
function testHiddenInternals(constructor, objType, excludeList) {
    ...
    properties.forEach(function (prop) {
        ...
        if (prop.indexOf("__", 0) === -1) {
            print("Detected additional property '" + prop + "' on '" + objType + "', if property is expected update this test's exclude lists.");
            extraProperties = true;
        }
    });
    if (extraProperties) {
        print("Failed for '" + objType + "'!");
    }
}
...
if(Intl.hasOwnProperty("EngineInterface") === true){
    print("EngineInterface object is not hidden.");
}
