...
print(`-wasmMaxTableSize:${(Math.pow(2,32)-1)|0}`);
...
print("testsuite/harness/sync_index.js");
  ...
  if (m) {
    ...
    if (isLib) {
      if (scriptToImport in mapping) {
        print(`testsuite/${mapping[scriptToImport]}`);
      } else {
        print(`testsuite/js-api/${scriptToImport}`);
      }
    } else {
      print(
        testfile
...
print(read("testsuite/harness/testharness.js")
...
function reportResult(tests, harness_status) {
  ...
  const testsReport = tests.map(test => {
    ...
    return `${status_text[test.status]} ${test.name} ${get_print(test)} ${test.message || ""}${stack ? "\n" + stack : stack}`;
  });
  print(`Harness Status: ${status_text_harness[harness_status.status]}
