...
function equal(ev, v) {
    ...
    if (sameValue(ev, v))
    {
        if(DEBUG) print("same value");
        return true;
    }
    else if ((ev == 0.0 || v == 0.0) && Math.abs(v - ev) <= eps) 
    {
        if(DEBUG) print("Float 0.0 ");
        return true;
    }
    else if (Math.abs(v - ev) / Math.abs(ev) <= eps)
    {
        if(DEBUG) print("Float values within eps");
        return true;
    }
    else
    {
        print(">> Fail!");
        print("Expected "+ev+" Found "+v);
        return false;
    }
}
