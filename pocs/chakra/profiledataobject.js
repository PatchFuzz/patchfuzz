...
function dumpProfileData(f, msg) {
    ...
    for (var i = 0; i < pdo.returnTypeInfo.length; ++i) {
        print("    #" + i + ": "  +  pdo.returnTypeInfo[i] + " (" + constantLookup(pdo.returnTypeInfo[i], pdo.ValueType) + ")");
    }
    
    if (pdo.elemInfo.length)
        print("Elem info:");
    for (var i = 0; i < pdo.elemInfo.length; ++i) {
        print("    #" + i + ": " + pdo.elemInfo[i] + " (" + constantLookup(pdo.elemInfo[i], pdo.ValueType) + ")");
    }

    if(pdo.parameterInfo.length)
        print("Param info:")
    for (var i = 0; i < pdo.parameterInfo.length; ++i) {
        print("    #" + i + ": " + pdo.parameterInfo[i] + " (" + constantLookup(pdo.parameterInfo[i], pdo.ValueType) + ")");
    }

    print("Implicit call flags:");
    print("    #" + i + ": " + pdo.implicitCallFlags + " (" + constantLookup(pdo.implicitCallFlags, pdo.ImplicitCallFlags) + ")");

    if(pdo.loopImplicitCallFlags.length)
        print("Loop implicit call flags:");
    for (var i = 0; i < pdo.loopImplicitCallFlags.length; ++i) {
        print("    #" + i + ": "  +  pdo.loopImplicitCallFlags[i] + " (" + constantLookup(pdo.loopImplicitCallFlags[i], pdo.ImplicitCallFlags) + ")");
    }
