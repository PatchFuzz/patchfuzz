...
{
    print("starting Property[",i,"]: ",propertyArray[i]);
    ...
    {
        if(isPropConfig) print("Fail");
        else print("PASS, Not Configurable and will not allow redefinition");
    }
    var deleteProp = false;
    if(isPropConfig)
    {
        if(RegExp[propertyArray[i]] =="OVERRIDE") print("PASS");
        else print("FAIL, currently equals: ",RegExp[propertyArray[i]]);
    }
    else
    {
        if(RegExp[propertyArray[i]] ==propValueBefore) print("PASS");
        else print("FAIL, currently equals: ",RegExp[propertyArray[i]]);
    }
    deleteProp = delete RegExp[propertyArray[i]];

    print("can you delete the property: ",canDelete, "did it actually delete?",deleteProp);
    if(deleteProp ==canDelete) print("Pass Delete Test");
...
function RegexInstanceTests(i,propertyArray)
{
    ...
    if(isPropConfig)
    {
        if(pattern1[propertyArray[i]] =="OVERRIDE") print("PASS");
        else print("FAIL, currently equals: ",pattern1[propertyArray[i]]);
    }
    else
    {
        if(pattern1[propertyArray[i]] ==propValueBefore) print("PASS");
        else print("FAIL, currently equals: ",pattern1[propertyArray[i]]);
    }
