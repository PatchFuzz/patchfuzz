...
const tests = [
    {
        ...
        body() {
            print(1, Uint8Array.prototype.sort.length, "Uint8Array.prototype.sort should have length of 1");
            print("sort", Uint8Array.prototype.sort.name, "Uint8Array.prototype.sort should have name 'sort'");
            const desc = Object.getOwnPropertyDescriptor(Uint8Array.prototype.__proto__, "sort");
            print(desc.writable, "Uint8Array.prototype.sort.writable === true");
            print(desc.enumerable, "Uint8Array.prototype.sort.enumerable === false");
            print(desc.configurable, "Uint8Array.prototype.sort.configurable === true");
            print('function', typeof desc.value, "typeof Uint8Array.prototype.sort === 'function'");
            print(() => { [].sort("not a function"); }, TypeError);
            print(() => { [].sort(null); }, TypeError);
            print(() => { [].sort(undefined); }, "Uint8Array.prototype.sort with undefined sort parameter does not throw");
        }
    },
    {
        ...
        body() {
            ...
            print(Uint8Array.from([2, 3, 4, 5, 6, 8, 9, 9, 10, 120]), arrayOne.slice().sort(undefined), "Uint8Array.sort with default comparator should sort based on numerical ordering");
            const result = Uint8Array.from([2, 3, 4, 5, 6, 8, 9, 9, 10, 120]);
            print(result, arrayOne.sort((x, y) => x - y), "Uint8Array.sort with numerical comparator should sort numerically");
            print(result, arrayOne, "Uint8Array.sort should sort original array as well as return value");
            ...
            print(Uint8Array.from([41, 25, 8, 7]), arrayTwo.sort((a, b) => b - a), "Uint8Array.sort with (a,b)=>b-a should sort descending");
            print(Uint8Array.from([7, 8, 25, 41]), arrayTwo.sort((a, b) => a - b), "Uint8Array.sort with (a,b)=>a-b should sort ascending");
        }
    },
    {
        ...
        body() {
            ...
            print(arrayOne.sort(compare), Uint16Array.from([
            ...
            print(arrayTwo.sort(compare), Uint16Array.from([
    ...
    },
    {
        ...
        body() {
            ...
            print(5, xyz, "Uint8Array.sort does not call compare function when length is 0");
            Uint8Array.from([1]).sort(setXyz)
            print(5, xyz, "Uint8Array.sort does not call compare function when length is 1");
            Float32Array.from([NaN, NaN, NaN, NaN]).sort(setXyz);
            print(5, xyz, "Float32Array.sort does not call compare function when all elements are NaN");
            Float32Array.from([5, NaN, , NaN]).sort(setXyz);
            print(5, xyz, "Float32Array.sort does not call compare function when only one element is defined");
            Float32Array.from([1, 2, NaN]).sort(setXyz);
            print(10, xyz, "Float32Array.sort calls compare function if there is > 1 defined element");
            const arr = Uint8Array.from([1, 0]);
            print(Uint8Array.from([0, 1]), arr.sort((a, b) => {arr[0] = 5; return a - b;}), "Uint8Array.sort protects array from compare function side effects");
            print(function (){arr.sort(() => {ArrayBuffer.detach(arr.buffer); return -1;})}, TypeError, "Uint8Array.sort throws if compare function detaches buffer");
        }
    },
    {
        name : "TypedArray.prototype.sort with an invalid parameter",
        body() {
            print(()=>{Uint8Array.prototype.sort.call(Uint8Array.from([]),5);}, TypeError);
            print(()=>{Uint8Array.prototype.sort.call(5,5);}, TypeError);
            print(()=>{Uint8Array.prototype.sort.call([]);}, TypeError);
        }
    }
];
