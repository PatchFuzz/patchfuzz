...
function test() {
  print(F.isF(new F));
  print(!F.isF(F) && !F.isF(new M) && !F.isF({}));
  shouldThrowTypeError(() => F.isF(3));
  print(SF.isSF(SF));
  print(!SF.isSF(new SF) && !SF.isSF(SM) && !SF.isSF({}));
  shouldThrowTypeError(() => SF.isSF(3));

  print(M.isM(new M));
  print(!M.isM(M) && !M.isM(new F) && !M.isM({}));
  shouldThrowTypeError(() => M.isM(3));
  print(SM.isSM(SM));
  print(!SM.isSM(new SM) && !SM.isSM(SF) && !SM.isSM({}));
  shouldThrowTypeError(() => SM.isSM(3));

  print(G.isG(new G));
  print(!G.isG(G) && !G.isG(new F) && !G.isG({}));
  shouldThrowTypeError(() => G.isG(3));
  print(SG.isSG(SG));
  print(!SG.isSG(new SG) && !SG.isSG(SF) && !SG.isSG({}));
  shouldThrowTypeError(() => SG.isSG(3));

  print(S.isS(new S));
  print(!S.isS(S) && !S.isS(new F) && !S.isS({}));
  shouldThrowTypeError(() => S.isS(3));
  print(SS.isSS(SS));
  print(!SS.isSS(new SS) && !SS.isSS(SF) && !SS.isSS({}));
