...
function CheckWith(orig, index, indexNotEvil, v, vNotEvil) {
  ...
  print(origLen, copy.length);
  for (let i = 0; i < origLen; i++) {
    print(i == absoluteIndex ? vNotEvil : orig[i], copy[i]);
  }
  print(origVAtIndex, orig[absoluteIndex]);
  print(copy === orig);
  print(copy.buffer === orig.buffer);
}
...
function CheckWithShrunkOrDetached(orig, v, vNotEvil) {
  ...
  if (orig instanceof BigInt64Array ||
    ...
    print(() => { orig.with(1, v); }, TypeError);
    return;
  }

  const copy = orig.with(1, v);
  print(origLen, copy.length);
  print(NormalizeValue(copy, 0), copy[0]);
  print(vNotEvil, copy[1]);
...
(function TestSurface() {
  for (let TA of ctors) {
    print(2, TA.prototype.with.length);
    print("with", TA.prototype.with.name);
  }
})();
...
(function TestOutOfBounds() {
  for (let TA of ctors) {
    let a = new TA(4);
    const v = NormalizeValue(a, 42);
    print(() => { a.with(a.length, v); }, RangeError);
    print(() => { a.with(-a.length - 1, v); }, RangeError);
  }
})();

(function TestNonTypedArray() {
  for (let TA of ctors) {
    print(() => { TA.prototype.with.call([1,2,3,4], 1, 42); }, TypeError);
  }
})();
...
(function TestResizableBuffer() {
  for (let TA of ctors) {
    ...
    print(() => { fixedLength.with(0, v); }, TypeError);
    print(() => { fixedLengthWithOffset.with(0, v); }, TypeError);
    CheckWith(lengthTracking, 0, 0, v, v);
    print(() => { lengthTrackingWithOffset.with(0, v); }, TypeError);
    ...
    print(() => { fixedLength.with(0, v); }, TypeError);
    print(() => { fixedLengthWithOffset.with(0, v); }, TypeError);
    print(() => { lengthTracking.with(0, v); }, RangeError);
    print(() => { lengthTrackingWithOffset.copyWithin(0, 1, 1); },
                 TypeError);
...
(function TestParameterConversionShrinks() {
  for (let TA of ctors) {
    ...
    const evilIndex = { valueOf: () => { rab.resize(2 * TA.BYTES_PER_ELEMENT);
                                          return 2; }};
    print(() => { fixedLength.with(evilIndex, v); },
                 RangeError);
...
(function TestParameterConversionDetaches() {
  for (let TA of ctors) {
    ...
    const evilV = { valueOf: () => { $.detachArrayBuffer(rab);
                                      return v; }};
    print(() => { fixedLength.with(1, evilV); }, TypeError);
  }
})();
...
(function TestNoSpecies() {
  ...
  print(Uint8Array, (new MyUint8Array(4)).with(0, 42).constructor);
})();
