...
dumpPaths(paths);
print(paths[0].length <= 5, true, "Too many paths reported");

paths = shortestPaths([f, g]);
print(paths.length, 2, "Two sets of paths expected");

paths = shortestPaths([f], {maxNumPaths: 1});
print(paths[0].length, 1, "Single path expected");
...
if (!getBuildConfiguration('pbl')) {
    ...
    print(e.includes("TypeError") && e.includes("1 argument required"), true);

    try { paths = shortestPaths(100, {}); } catch (exc) { e = ""+exc; };
    print(e, "TypeError: 100 is not an array object");

    try { paths = shortestPaths([f], {start: 200}); } catch (exc) { e = ""+exc; };
    print(e, "TypeError: 200 is not a GC thing");
    ...
    print(e, "TypeError: -1 is not greater than 0");

    
    let arr = [{}];
    let objWithGetter = {get start() { arr.length = 0; return {}; }};
    try { paths = shortestPaths(arr, objWithGetter); } catch (exc) { e = ""+exc; }
    print(e, "TypeError: arr is not a dense array object with one or more elements");
}
