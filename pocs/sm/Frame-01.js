...
dbg.onDebuggerStatement = function (f) {
    print(Object.getPrototypeOf(f), Debugger.Frame.prototype);
    print(f.type, expected.type);
    print(f.script.isGeneratorFunction, expected.generator);
    print(f.constructing, expected.constructing);
    hits++;
};
...
function test(code, expectobj, expectedHits) {
    ...
    hits = 0;
    g.evaluate(code);
    print(hits, arguments.length < 3 ? 1 : expectedHits);
}
