...
function monitorGlobalValues(wast, lib, expected) {
    ...
    print(framesGlobals.length, expected.length);
    for (var i = 0; i < framesGlobals.length; i++) {
        ...
        print(globalsNames.length, Object.keys(expectedGlobals).length);
        globalsNames.forEach(n => {
            if (typeof expectedGlobals[n][0] === "function") {
                
                expectedGlobals[n].forEach((assertFn, i) => {
                    print(frameGlobals[n][i]);
                });
                return;
            }
            print(frameGlobals[n], expectedGlobals[n]);
        });
    }
}
    ...
    [{
        memory0: [
            function (actual) {
                ...
                print(!!bufferProp, true, "wasm memory buffer property");
                ...
                print(bufferLength, 65536, "wasm memory size");
            }
        ]
    }]
);
monitorGlobalValues(
    ...
    [(function () {
        let x = {
            ...
            global2: [3.5],
            global3: [42.25],
            global4: [ function (x) { print(x.optimizedOut, true); } ],
        };
        return x;
    })()]
);
