...
eval(`let x = 42; print(x, 42);`);
eval(`var let = 42; print(let, 42);`);
eval(`let;`);
eval(`[...let] = [];`);
eval(`function let() { return 42; } print(let(), 42);`)
eval(`let {x:x} = {x:42}; print(x, 42);`);
eval(`let [x] = [42]; print(x, 42);`);

eval(`for (let x in [1]) { print(x, "0"); }`);
eval(`for (const x in [1]) { print(x, "0"); }`);

eval(`for (let x of [1]) { print(x, 1); }`);
eval(`for (const x of [1]) { print(x, 1); }`);

eval(`for (let i = 0; i < 1; i++) { print(i, 0); }`);
eval(`var done = false; for (const i = 0; !done; done = true) { print(i, 0); }`);

eval(`for (let of of [1]) { print(of, 1); }`);
eval(`for (const of of [1]) { print(of, 1); }`);

eval(`try { throw 17; } catch (let) { print(let, 17); }`);
eval(`try { throw [17]; } catch ([let]) { print(let, 17); }`);
eval(`try { throw { x: 17 }; } catch ({ x: let }) { print(let, 17); }`);
eval(`try { throw {}; } catch ({ x: let = 17 }) { print(let, 17); }`);

expectError(`try { throw [17, 42]; } catch ([let, let]) {}`);

eval(`for (let in [1]) { print(let, "0"); }`);
