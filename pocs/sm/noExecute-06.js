...
function testHook(hookName) {
  ...
  for (var s of strs) {
    ...
    print(g1w1.executeInGlobal(s).throw.unsafeDereference() instanceof Debugger.DebuggeeWouldRun, true);
    print(g1w2.executeInGlobal(s).throw.unsafeDereference() instanceof Debugger.DebuggeeWouldRun, true);
    if (newestG1Frame) {
      print(newestG1Frame.eval(s).throw.unsafeDereference() instanceof Debugger.DebuggeeWouldRun, true);
    }
    print(fw1.apply(null).throw.unsafeDereference() instanceof Debugger.DebuggeeWouldRun, true);

    
    print(g2w.executeInGlobal(s).throw, undefined);
    if (newestG2Frame) {
      print(newestG2Frame.eval(s).throw, undefined);
    }
    print(fw2.apply(null).return, 42);
  }
}
