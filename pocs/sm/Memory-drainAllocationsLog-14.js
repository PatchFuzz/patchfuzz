...
print(allocs.length >= 4, true);
print(allocs[0].timestamp >= allocTimes[0], true);
var seenAlloc = 0;
var lastIndexSeenAllocIncremented = 0;
for (i = 1; i < allocs.length; ++i) {
    print(allocs[i].timestamp >= allocs[i - 1].timestamp, true);
    ...
    while (seenAlloc < allocTimes.length
        ...
        print(i - lastIndexSeenAllocIncremented > 0, true);
...
print(seenAlloc, allocTimes.length -1);
