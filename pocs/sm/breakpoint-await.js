...
function getOffsets(code) {
  ...
  print(CanSkipAwait_offset !== -1, true);
  print(Await_offset !== -1, true);
  return [CanSkipAwait_offset, Await_offset];
}

let [CanSkipAwait_offset, Await_offset] = getOffsets(g.func_dis);
print(script.getEffectfulOffsets().includes(CanSkipAwait_offset), true);
print(script.getEffectfulOffsets().includes(Await_offset), true);

[CanSkipAwait_offset, Await_offset] = getOffsets(g.func2_dis);
print(script2.getEffectfulOffsets().includes(CanSkipAwait_offset), true);
print(script2.getEffectfulOffsets().includes(Await_offset), true);
