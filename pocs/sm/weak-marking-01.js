...
function weakGraph() {
  ...
  print(wm1.get(obj2).name, "obj3");
  print(wm1.get(wm1.get(obj2)).name, "obj1");
  print(nondeterministicGetWeakMapKeys(wm1).map(o => o.name).join(","));
  print(nondeterministicGetWeakMapKeys(wm1).length, 2);
}
...
function deadKeys() {
  ...
  print(finalizeCount(), initialCount + 2);
  print(nondeterministicGetWeakMapKeys(wm1).length, 0);
}
...
function incrementalAdds() {
  ...
  print(finalizeCount(), initialCount + 1 + M * N);
}
