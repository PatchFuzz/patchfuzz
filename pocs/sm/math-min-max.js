function test1ArgInt32() {
    function test(x, expected) {
        print(Math.max(x), expected);
        print(Math.min(x), expected);
    }
    for (var i = 0; i < 20; i++) {
        test(i, i);
    }
...
function test2ArgsInt32() {
    function test(x, y, expectedMax, expectedMin) {
        print(Math.max(x, y), expectedMax);
        print(Math.min(x, y), expectedMin);
    }
    for (var i = 0; i < 20; i++) {
        test(0, i, i, 0);
        test(-9, -1, -1, -9);
...
function test3ArgsInt32() {
    function test(a, b, c, expectedMax, expectedMin) {
        print(Math.max(a, b, c), expectedMax);
        print(Math.min(a, b, c), expectedMin);
    }
    for (var i = 0; i < 20; i++) {
        test(30, 100, i, 100, i);
        test(i, 0, -2, i, -2);
...
function test4ArgsInt32() {
    function test(a, b, c, d, expectedMax, expectedMin) {
        print(Math.max(a, b, c, d), expectedMax);
        print(Math.min(a, b, c, d), expectedMin);
    }
    for (var i = 0; i < 20; i++) {
