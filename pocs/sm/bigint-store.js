...
function testStore() {
  ...
  for (let i = 0; i < 50; ++i) {
    for (let j = 0; j < bigIntValues.length; ++j) {
      ...
      print(Atomics.store(int64, 0, value), value);
      print(int64[0], BigInt.asIntN(64, value));

      print(Atomics.store(uint64, 0, value), value);
  ...
  for (let i = 0; i < 50; ++i) {
    for (let j = 0; j < bigIntValues.length; ++j) {
      ...
      print(Atomics.store(int64, idx, value), value);
      print(int64[idx], BigInt.asIntN(64, value));

      print(Atomics.store(uint64, idx, value), value);
