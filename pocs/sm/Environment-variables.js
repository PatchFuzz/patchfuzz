...
function test(code, debugStmts, followupStmts) {
    ...
    dbg.onDebuggerStatement = function (frame) {
        ...
        print(env.getVariable("x"), val)
        print(env.setVariable("x", 'ok'), undefined);
        print(env.getVariable("x"), 'ok');

        
        print(function () { env.setVariable("newVar", 0); }, TypeError);
        hits++;
    };
...
for (var s of cases) {
    ...
    test(s, "eval(\"\"); debugger; print(x, 'ok');");

    
    test(s, "debugMe(); print(x, 'ok');");

    
    test(s, "{ let y = 'irrelevant'; (function (z) { { let zz = y; eval(\"\"); debugger; } })(); } print(x, 'ok');"),

    
    
    test(s, "capture = {dbg: function () { eval(\"\"); debugger; }, get x() { return x; }};",
            "print(capture.x, VAL); capture.dbg(); print(capture.x, 'ok');");
}
