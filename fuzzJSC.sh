...
TARGET_PATH="/home/WebKit/0411/Release/bin/jsc"
...
for ((i=1; i<=PARALLE_NUMBER; i++)); do
    ...
    if [ $i -eq 1 ];then
    	eval "nohup ${FUZZER_DIR}/afl-fuzz -M jsc_1 -m 4G -t 600 -i ${PARENT_DIR}/copy_1 -o ${FUZZOUT_DIR} ${TARGET_PATH} ${ARGS} @@ > /dev/null 2>&1 &" 
    else
